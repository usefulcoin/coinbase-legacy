...
async function makebid ( bidprice, bidquantity, configurationinformation ) {
  ...
  let bidprice = Number( bidprice ) + Number( quoteincrement ); /* always add the quote increment to ensure that the bid is the most competitive bid */
  let bidquantity = Number( quoteriskablebalance ) / Number( bidprice ); /* defined safe (riskable) bid quantity not exceeding the balance/price amount */
...
(async function main() {
  ...
  ws.on('message', async function incoming(data) { // start handling websocket messages.
    ...
    if ( jsondata.type === 'snapshot' ) { // handle level2 snapshot message.
      ...
      else { // capture the first bid from the pile of bids in the snapshot.

        let snapshotprice = jsondata.bids[0][0]; /* capture best ask price from the orderbook. */
        let snapshotsize = jsondata.bids[0][1]; /* capture best ask quantity from the orderbook. */
