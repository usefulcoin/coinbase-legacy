...
if [ $# -eq 7 ]; then
	...
	spread=$4 && echo [ $(date) ] setting spread = $spread
	return=$5 && echo [ $(date) ] setting return = $return
	product=$6 && echo [ $(date) ] setting product = $product
	recipient=$7 && echo [ $(date) ] setting recipient = $recipient
else
	...
	spread=0.01 && echo [ $(date) ] setting spread = $spread
	return=0.05 && echo [ $(date) ] setting return = $return
...
node index.js "$websocketserver" "$restapiserver" "$riskratio" "$spread" "$return" "$product" "$recipient"
